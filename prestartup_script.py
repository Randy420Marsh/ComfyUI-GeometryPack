...
# ---------------------------------------------------------------------------
# Crash diagnostics: install hooks that persist AFTER prestartup completes.
# These help identify which ComfyUI import causes the process to crash.
# Only active when COMFY_ENV_DEBUG=1 (set in CI).
# ---------------------------------------------------------------------------
            ...
            if fullname not in sys.modules:
                ...
                if parts[0] in self._PREFIXES and len(parts) <= 2:
                    print(f"[import-trace] {fullname}",
                          file=sys.stderr, flush=True)
            return None  # never handle — let real finders do the work
